...
Что он различает — шесть состояний, а не два:
...
* **недоступна с этой машины** — соединение не устанавливается вовсе,
  а страница при этом отдаётся с другой точки выхода. Реестр
  `UNREACHABLE_FROM_HERE` ниже, каждая строка обязана называть
  измерение;
* **локальный адрес** — `127.0.0.1`/`localhost`, адрес собственного
  приложения `app/` (решение 40), а не внешнего ресурса;
* **мёртвая** — 404, 410, 5xx, отказ соединения, и ни в одном из
  реестров её нет. Только она даёт код возврата 1.
...
# Пятое состояние: жива снаружи, но недоступна с машины автора. Не
# синоним «закрыта для автозапросов» (там отвечает сервер — 401/403) и не
# мёртвая: TCP-соединение не устанавливается вовсе, а страница при этом
# отдаётся с другой точки выхода. Реестр адресный, а не доменный, и
# каждая строка обязана называть измерение, иначе он превращается в
# способ прятать мёртвые ссылки.
#
# Измерено 2026-09-03: `nslookup protocol.ua` → 91.205.18.92 (DNS жив),
# `curl --max-time 20` по 443 и по 80 → «Connection timed out», код 000
# оба раза; внешний fetch того же адреса в тот же день вернул страницу
# «Стаття 293. Ставки єдиного податку» с дословными «3 відсотки доходу»
# и «5 відсотків доходу» — тем самым текстом, на который ссылается
# `research/access.md`. То есть недоступен путь, а не источник.
UNREACHABLE_FROM_HERE = {
    "https://protocol.ua/ua/podatkoviy_kodeks_ukraini_stattya_293/":
        "недоступен с этой машины (TCP timeout по 443 и 80), проверен "
        "внешним fetch 2026-09-03: цитата ст. 293.3 на месте",
}
...
def main() -> int:
    ...
    offnet = {
        u: f for u, f in urls.items()
        if u not in skipped and u not in local and u in UNREACHABLE_FROM_HERE
    }
    to_check = {
        u: f for u, f in urls.items()
        if u not in skipped and u not in local and u not in offnet
    }

    print(f"Ссылок в *.md: {len(urls)}; проверяется {len(to_check)}, "
          f"пропущено по решению 36: {len(skipped)}, локальных: {len(local)}, "
          f"недоступных с этой машины: {len(offnet)}\n")
    ...
    for url in sorted(offnet):
        print(f"[ВНЕ СЕТИ] {url} — {UNREACHABLE_FROM_HERE[url]}")
