...
def warn(msg: str) -> None:
    """Предупреждение: печатается, но не роняет прогон. Для случаев, где
...
# настройка окружения. Умения не несёт и в покрытии не участвует, но пометка
# обязательна — шаг без строки "Умение:" вовсе считается дефектом, а не
# третьим молчаливым вариантом.
...
def check_step_skill_header() -> None:
    ...
    и инфраструктурные шаги приходилось приписывать к умению: step-09.md
...
def check_skill_ids(blueprint_text: str) -> None:
    ...
    # регулярного выражения: они ничего не закрывают (решение 23).
...
def check_module_hours(blueprint_text: str) -> None:
    ...
    Решение 24: сумма шагов — факт, вилка — оценка; при расхождении правится
    вилка, а не часы в шагах. Отсюда две ситуации, которые проверка обязана
    называть вслух, и обе — [WARN], не [FAIL]: обе возможны честно, но обе
    обязаны быть замечены.
    ...
    стиль сведения»). Подогнанная сумма уничтожает данные для калибровки по
    research/self.md до того, как они появятся.

    (б) Сумма целиком вне вилки — то, что случилось с M3 (37–47 при 60–80).
    Это сигнал сначала искать недостающее содержание (там его и нашли:
    шаги 09–12 довели сумму до 60–77), и только потом править вилку."""
    if not PROGRAM_DIR.exists():
        return
    ...
    for d in module_dirs:
        ...
        if no_hours:
            fail(f"{d.name}: шаги без строки 'Время: N{DASH}M ч': {', '.join(no_hours)}")
            continue
        if not pairs:
            continue
