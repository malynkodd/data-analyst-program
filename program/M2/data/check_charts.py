...
5. два экспортированных агрегата (`work\\chart_quarter.csv`,
   ...
   весят не меньше 1 КБ (порог — см. `MIN_PNG_BYTES` ниже).
...
MIN_MATCH = 5
MIN_REASON_WORDS = 5
# Порог отделяет пустой экспорт от настоящего графика, и меряется он
# прогоном, а не на глаз. Замер 2026-09-05, Excel Microsoft 365 ru-ru,
# штатный Chart.Export в PNG: пустая диаграмма без данных — 307 байт при
# размере по умолчанию (360x216) и 550 байт при 480x300; точечная
# диаграмма с одной точкой — 3870 байт, с восемью точками — 5038 байт.
# Прежний порог 5 КБ отбраковывал настоящую точечную диаграмму,
# экспортированную с размером по умолчанию (симуляция 2026-09-04,
# замечание M2-6): между «пусто» и «самая разреженная из шести форм»
# лежит промежуток 550...3870, и порог ставится в него.
MIN_PNG_BYTES = 1024
...
def check_images() -> None:
    ...
    if small:
        fail(f"картинки меньше {MIN_PNG_BYTES} байт (пустой экспорт): " + ", ".join(small))
