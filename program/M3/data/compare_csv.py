...
3. **Десятичная запятая и разделитель разрядов.** Тот же скрипт сверяет
   экспорт визуала Power BI (решение 22 `design/decisions.md`), а Power BI
   на русской локали печатает `2480,00` там, где SQL печатает `2480.00`,
   и берёт такое значение в кавычки, потому что запятая совпала с
   разделителем полей. Проверено прогоном, `research/tools-gate.md`,
   находка P9: до правки скрипт объявлял это расхождением, хотя данные
   совпадали. Разряды при этом могут быть разделены пробелом, в том числе
   неразрывным (`1 234,56`).

Скрипт сравнивает **значения**, разобранные по правилам CSV, а не байты,
поэтому все три пункта выше на результат не влияют. Числа сравниваются как
числа: `1200.0` и `1200` — совпадение, `1200.0` и `1200.5` — нет.

Нормализация числа не превращает разные числа в одинаковые — это
проверяется `test_compare_csv.py` в той же папке: `2480,00` против
`2490.00` остаётся расхождением, а `1,234` против `1234` — тоже
расхождением, потому что одна запятая в записи читается как десятичная, а
не как разделитель разрядов.
"""
...
def normalize_number(text: str) -> str:
    """Приводит запись числа к тому виду, который принимает float().

    Снимает два наслоения локали, оба измерены прогоном
    (`research/tools-gate.md`, P9): разделитель разрядов пробелом и
    десятичную запятую.

    Запятая заменяется на точку **только когда она в записи одна**.
    `1,234` при этом читается как одна целая двести тридцать четыре
    тысячных, а не как тысяча двести тридцать четыре: угадывать между
    двумя несовместимыми соглашениями скрипт не имеет права, и из двух
    вариантов выбран тот, при котором `1,234` не совпадёт с `1234` и
    расхождение будет напечатано. Запись с двумя и более запятыми
    (`1,234.56`) числом не признаётся вовсе — float() на ней падает, и
    значения сравниваются как строки.

    Три пробела в кортеже ниже — обычный (U+0020), неразрывный (U+00A0) и
    узкий неразрывный (U+202F). В исходнике они визуально неразличимы;
    что там действительно три разных символа, проверяет
    `test_compare_csv.py`.
    """
    for space in (" ", " ", " "):
...
def same_value(got: str, want: str) -> bool:
    if got == want:
        return True
    try:
        return abs(float(normalize_number(got)) - float(normalize_number(want))) < 1e-9
    except ValueError:
        return False
