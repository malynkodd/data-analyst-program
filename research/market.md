...
привязки к конкретной стране.**

**База числа (решение 30 `design/decisions.md`).** Строк со значением
«**Да (явно)**» в колонке «UA/B2B» — **четыре**: 7 (ProviderNow), 8 (At
Your School), 15 (Peroptyx), 17 (Ruby Labs). Публикуемое «3 из 21»
считает из них только **позиции аналитика данных**: вакансия 15
(Peroptyx) — почасовой краудворк по оценке карт для обучения ИИ
(«Anywhere in the World», вилка $10k–25k), и в её же ячейке это
записано словами «не классический DA». Строк «нет данных» в колонке
«UA/B2B» нет ни одной — все 21 классифицированы явно; «нет данных»
встречается только в колонке «Тестовое» (16 строк). Итого два числа с
разными базами: **3 из 21 (14 %)** — открытые позиции аналитика,
**4 из 21 (19 %)** — все строки с «Да». Дальше по репозиторию
используется первое. Из них одна ([Ruby Labs](https://jobs.ashbyhq.com/ruby-labs/02eda1b4-c84a-4ee8-8a3d-0fdcc93cf0a9))
...
конкретной стране** (база — позиции аналитика данных; строк с «Да» в
колонке четыре, четвёртая — краудворк, см. «База числа» выше). Из 18
строк, не дающих открытой позиции аналитика (17 закрытых явно + 1
краудворк), закрытые почти все ограничены явным списком
